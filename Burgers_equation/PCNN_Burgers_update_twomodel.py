...
modelu = MLP(3, [30, 20, 30, 20], 1).to(device)
modelv = MLP(3, [30, 20, 30, 20], 1).to(device)

# Initialize weights
for param_tensor in modelu.state_dict():
    if modelu.state_dict()[param_tensor].dim() > 1:
        nn.init.xavier_normal_(modelu.state_dict()[param_tensor])
        nn.init.xavier_normal_(modelv.state_dict()[param_tensor])
...
def uv(t, x, y):
    u = modelu(torch.cat((t,x,y),1))
    v = modelv(torch.cat((t,x,y),1))
    #if t.eq(0.0).all():
    #    u_v[:,0] = uv0(x,y)[:,0]
    #    u_v[:,1] = uv0(x,y)[:,1]
...
MSE = nn.MSELoss()
optimizer1 = torch.optim.Adam(modelu.parameters(), lr=learning_rate, weight_decay=1e-6)
scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer1, step_size=20000, gamma=0.2) #(optimizer1, step_size=100, gamma=0.97)# # 0.0005 -> 0.0001 -> 0.00002 -> 0.000004 -> 0.0000008
optimizer2 = torch.optim.Adam(modelv.parameters(), lr=learning_rate, weight_decay=1e-6)
scheduler2 = torch.optim.lr_scheduler.StepLR(optimizer2, step_size=20000, gamma=0.2)
...
for epoch in range(num_epochs):
    ...
    cost =  ( cost3**2 + cost2 ** 2 + cost4**2) / ( cost3 + cost2 + cost4 )
...
torch.save(modelu.state_dict(), './Burgers_equation/PCNN_burgersu.pkl')
torch.save(modelv.state_dict(), './Burgers_equation/PCNN_burgersv.pkl')
...
modelu.load_state_dict(torch.load('./Burgers_equation/PCNN_burgersu.pkl'))
modelv.load_state_dict(torch.load('./Burgers_equation/PCNN_burgersv.pkl'))
def uv(t, x, y):
    u = modelu(torch.cat((t,x,y),1))
    v = modelv(torch.cat((t,x,y),1))
    #if t.eq(0.0).all():
