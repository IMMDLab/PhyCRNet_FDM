'''PhyCRNet for solving burgers PDEs-06/03/2024'''
...
class loss_generator(nn.Module):
    ...
    def get_heat_loss(self, output, u0, all_img, type):
        ...
        u = output[:, 0:1, :, :,]  #  training time is 0-1 seconds
        lent = u.shape[0]
        lenx = u.shape[3]
        leny = u.shape[2]
        ...
        v = output[:, 1:2, :, :,]  #  training time is 0-1 seconds
        lent = v.shape[0]
        lenx = v.shape[3]
        leny = v.shape[2]
        ...
        if type == 'phy':
            ...
            fu = u_t[:, :, 1:-1, 1:-1] - 0.005*laplace_u + u[1:-1, :, 1:-1, 1:-1] * u_x[1:-1] + v[1:-1, :, 1:-1, 1:-1] * u_y[1:-1]
            fv = v_t[:, :, 1:-1, 1:-1] - 0.005*laplace_v + u[1:-1, :, 1:-1, 1:-1] * v_x[1:-1] + v[1:-1, :, 1:-1, 1:-1] * v_y[1:-1]
            ...
            " mixed BCs"
            fu_x1 = u[1:-1,:,:,0] # Left BC
            fu_x2 = u[1:-1,:,:,-1] # Right BC
            fu_y1 = u_b[1:-1]
            fu_y2 = u_u[1:-1]

            fv_x1 = v_l[1:-1] 
            fv_x2 = v_r[1:-1]
            fv_y1 = v[1:-1,:,0,:] # Bottom BC
            fv_y2 = v[1:-1,:,-1,:] # Top BC

            x1 = 0.5*torch.cos(4*torch.pi*y).cuda()
            x2 = 0.5*torch.cos(4*torch.pi*y).cuda()
            y1 = 0.5*torch.cos(4*torch.pi*x).cuda()
            y2 = 0.5*torch.cos(4*torch.pi*x).cuda()
            ...
            f1 = ((fu_x1-x1_d)**2 + (fu_x2-x2_d)**2 + (fu_y1)**2 + (fu_y2)**2).mean()
            f2 = ((fv_x1)**2 + (fv_x2)**2 + (fv_y1-y1_d)**2 + (fv_y2-y2_d)**2).mean()
            ...
            fu = (u[0].squeeze()-(0.5*(torch.sin(4*torch.pi*x).to(device)+torch.cos(4*torch.pi*y).to(device))).T) # Add .T because meshgrid default left top as 0,0
            fv = (v[0].squeeze()-(0.5*(torch.cos(4*torch.pi*x).to(device)+torch.sin(4*torch.pi*y).to(device))).T) # but the image is left bottom as 0,0
...
def compute_loss(output, u0, all_img, loss_func):
    ...
    return f, f_phy/100, f_bc/100, f_ic, f_data
...
if __name__ == '__main__':
    ...
    fig_save_path = './Burgers_equation/'
    data = pd.read_csv('./Data/burgers_update1_100%.csv', skiprows=0)  # read comsol generated data
    ...
    # get all image
    all_imgs = []
    data_sample = pd.read_csv('./Data/burgers_update1_100%.csv', skiprows=0)  # read comsol generated data
    ...
    model_save_path =  './Burgers_equation/phycrnet_burgers.pt' # '/home/gli12/project/PhyCRNet/phyCRNet_AM/checkpoint_heat.pt'
    ...
    fig_save_path = './Burgers_equation/' #'/home/gli12/project/PhyCRNet/phyCRNet_AM/result_figure/'
    ...
    plotfigure_uv(X, Y, ZU_truth0.flatten(), ZV_truth0.flatten(), 'Truth-Burgers', 0.0, './Burgers_equation/Results/')
    plotfigure_uv(X, Y, ZU_truth0_5.flatten(), ZV_truth0_5.flatten(), 'Truth-Burgers', 0.5, './Burgers_equation/Results/')
    plotfigure_uv(X, Y, ZU_truth1.flatten(), ZV_truth1.flatten(), 'Truth-Burgers', 1.0, './Burgers_equation/Results/')
    plotfigure_uv(X, Y, ZU_truth1_5.flatten(), ZV_truth1_5.flatten(), 'Truth-Burgers', 1.5, './Burgers_equation/Results/')
    plotfigure_uv(X, Y, ZU_truth2.flatten(), ZV_truth2.flatten(), 'Truth-Burgers', 2.0, './Burgers_equation/Results/')

    # extropolate from start
    data_t1 = np.array(data)[:,[0,1,42,43]].T
    imageu1, imagev1 = data_to_img_new(data_t1)
    imageuv1 = np.array([imageu1, imagev1])[np.newaxis, :]
    input = torch.tensor(imageuv1, dtype=torch.float32).to(device)
    output, _ = model(initial_state, input)  # shape of putput,  len - 21
    output = torch.cat(tuple(output), dim=0)  # make the output list as a tu]ensor
    output = torch.cat((input.to(device), output), dim=0)  # insert the input to output, input will be in the first of dim 0.
    output = output.cpu().detach().numpy()
    ...
    Mymodel = 'PhyCRNet_Burgers_update_start'
    u_test_err1, v_test_err1 = np.mean((output[21,0].squeeze()-ZU_truth2)**2), np.mean((output[21,1].squeeze()-ZV_truth1)**2) # t=1.0 seconds
    u_test_err0, v_test_err0 = np.mean((output[0,0].squeeze()-ZU_truth1)**2), np.mean((output[0,1].squeeze()-ZV_truth0)**2) # t=0.05 seconds
    u_test_err0_5, v_test_err0_5 = np.mean((output[11,0].squeeze()-ZU_truth1_5)**2), np.mean((output[11,1].squeeze()-ZV_truth0_5)**2) # t=0.5 seconds
    print('U ,V test_error t=0: ', np.sqrt(u_test_err0), np.sqrt(v_test_err0))
    print('U ,V test_error t=0.5:', np.sqrt(u_test_err0_5), np.sqrt(v_test_err0_5))
    print('U ,V test_error t=1: ', np.sqrt(u_test_err1), np.sqrt(v_test_err1))
    ZU1, ZV1 = output[21][0].flatten(), output[21][1].flatten()
