...
class loss_generator(nn.Module):
    ...
    def get_heat_loss(self, output, u0, all_img, type):
        ...
        if type == 'phy':
            ...
            # f_y2 = u_u[1:-1]
            "priodic BC"
            # fu_x1 = u[1:-1,:,:,0] - u[1:-1,:,:,-1]
            # fu_x2 = u_l[1:-1] - u_r[1:-1]
            # fu_y1 = u[1:-1,:,0,] - u[1:-1,:,-1,]
            # fu_y2 = u_b[1:-1] - u_u[1:-1]

            # fv_x1 = v[1:-1,:,:,0] - v[1:-1,:,:,-1]
            # fv_x2 = v_l[1:-1] - v_r[1:-1]
            # fv_y1 = v[1:-1,:,0,] - v[1:-1,:,-1,]
            # fv_y2 = v_b[1:-1] - v_u[1:-1]
            # fu = torch.mean((torch.square(fu_x1) + torch.square(fu_x2) \
            #     + torch.square(fu_y1) + torch.square(fu_y2))/(fu_x1.shape[-1]))   
            # fv = torch.mean((torch.square(fv_x1) + torch.square(fv_x2) \
            #     + torch.square(fv_y1) + torch.square(fv_y2))/(fv_x1.shape[-1]))
            " flux 0.1"
            fu_x1 = u_l[1:-1]
            fu_x2 = u_r[1:-1]
            fu_y1 = u_b[1:-1]
            ...
            fv_y1 = v_b[1:-1]
            fv_y2 = v_u[1:-1]
            f = torch.mean((torch.square(fu_x1+0.1) + torch.square(fu_x2+0.1) \
                + torch.square(fu_y1-0.1) + torch.square(fu_y2-0.1) + torch.square(fv_x1+0.1) + torch.square(fv_x2+0.1) \
                + torch.square(fv_y1-0.1) + torch.square(fv_y2-0.1))/(10*fu_x1.shape[-1]))   
            #f = fu + fv
            ...
            fu = torch.mean((u[0].squeeze()-0.5*(torch.sin(4*torch.pi*x).to(device)+torch.sin(4*torch.pi*y).to(device)))**2)
            fv = torch.mean((v[0].squeeze()-0.5*(torch.cos(4*torch.pi*x).to(device)+torch.cos(4*torch.pi*y).to(device)))**2)
            f = fu + fv

        elif type == 'data':
            output_s1 = output[:-1].flatten()
            all_img_s1 = all_img.squeeze().flatten()

            # sampling 5% of the data
            torch.manual_seed(42)
            randomlist = [torch.randperm(output_s1.size()[0])][:int(0.2*torch.numel(output_s1))]
            output_s1 = output_s1[randomlist[0]]#[torch.randperm(output_s1.size()[0])][:int(0.05*torch.numel(output_s1))]
            all_img_s1 = all_img_s1[randomlist[0]]#[torch.randperm(all_img_s1.size()[0])][:int(0.05*torch.numel(all_img_s1))]
...
def compute_loss(output, u0, all_img, loss_func):
    ...
    f = f_phy+f_bc+f_data+f_ic*10

    return f, f_phy/50, f_bc, f_ic*10, f_data
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('./Data/burgersuv_flux0.1.csv', skiprows=0)  # read comsol generated data
    ...
    # get all image
    all_imgs = []
    data_sample = pd.read_csv('./Data/burgersuv_flux0.1.csv', skiprows=0)  # read comsol generated data
    ...
    data0 = np.array(data)[:, [0,1,2,3]].T # t=0.05 seconds
