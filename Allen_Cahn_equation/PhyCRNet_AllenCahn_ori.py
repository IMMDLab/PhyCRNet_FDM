    ...
    def get_heat_loss(self, output, u0, all_img, type):
        ...
        if type == 'phy':
            ...
            f = (torch.square(f_x1) + torch.square(f_x2) \
                + torch.square(f_y1) + torch.square(f_y2)).mean()#/f_x1.shape[-1]
            ...
            f = (output[1]-0.5*(torch.sin(4*torch.pi*x).cuda()+torch.sin(4*torch.pi*y).cuda())).mean()
...
if __name__ == '__main__':
    ...
    fig_save_path = './Allen_Cahn_equation/'
    data = pd.read_csv('/home/gli12/project/ASME2024/Allen_Cahn_equation/allen_cahn.csv', skiprows=0)
    ...
    # get all image
    all_imgs = []
    data_sample = pd.read_csv('/home/gli12/project/ASME2024/Allen_Cahn_equation/allen_cahn.csv', skiprows=0)
    for i in range(time_steps):
        all_img = np.array(data_sample)[:,[0,1,1*i+3]]
    ...
    model_save_path =  './Allen_Cahn_equation/phycrnet_heat.pt' # '/home/gli12/project/PhyCRNet/phyCRNet_AM/checkpoint_heat.pt'
    ...
    fig_save_path = './Allen_Cahn_equation/' #'/home/gli12/project/PhyCRNet/phyCRNet_AM/result_figure/'
    data1 = np.array(data)[:, [0,1,22]].T   # t=1.0 seconds
    data0_5 = np.array(data)[:, [0,1,12]].T  # t=0.5 seconds
    data1_5 = np.array(data)[:, [0,1,32]].T  # t=1.5 seconds
    data2 = np.array(data)[:, [0,1,42]].T  # t=2.0 seconds
    ...
    # output_to_img(image_out, name='output_heat')

    # from training
    test_err1 = np.mean((image_out[-1].squeeze()-truth1)**2) # np.mean((image_out[-1].squeeze()-truth)**2)
    test_err0 = np.mean((image_out[1].squeeze()-image0)**2)
    test_err0_5 = np.mean((image_out[11].squeeze()-truth0_5)**2) 
    
    print('test_error t=0: ',  np.sqrt(test_err0))
    print('test_error t=0.5:', np.sqrt(test_err1))
    ...
    Z1 = output[-1][0].flatten()#.squeeze().cpu().detach().numpy().flatten()
    Z0 = output[1][0].flatten()
    Z0_5 = output[11][0].flatten()
    Z1_5 = output_extro[11][0].flatten()
    Z2 = output_extro[-1][0].flatten()
    Z_truth = truth1.flatten()
    image0 = image0.flatten()
    Mymodel = 'PhyCRNet_Allen_Cahn'
    ...
    plotfigure(X, Y, truth0.flatten(), Mymodel+'_Truth ', 0.0, './Allen_Cahn_equation/')
    plotfigure(X, Y, truth0_5.flatten(), Mymodel+'_Truth ', 0.5, './Allen_Cahn_equation/')
    plotfigure(X, Y, truth1.flatten(), Mymodel+'_Truth ', 1.0, './Allen_Cahn_equation/')
    plotfigure(X, Y, truth1_5.flatten(), Mymodel+'_Truth ', 1.5, './Allen_Cahn_equation/')
    plotfigure(X, Y, truth2.flatten(), Mymodel+'_Truth ', 2.0, './Allen_Cahn_equation/')
    ...
    # plt.xlim(0.0, 1.0)
    # plt.ylim(0.0, 1.0)
    # plt.savefig('/home/gli12/project/PhyCRNet/phycrnet_heat/' + Mymodel + '-heat' + str(1) + '.png')
    # plt.show()
    # plt.close()
    ...
    # plt.xlim(0.0, 1.0)
    # plt.ylim(0.0, 1.0)
    # plt.savefig('/home/gli12/project/PhyCRNet/phycrnet_heat/' + Mymodel + '-heat' + str(0) + '.png')
    # plt.show()
    # plt.close()
