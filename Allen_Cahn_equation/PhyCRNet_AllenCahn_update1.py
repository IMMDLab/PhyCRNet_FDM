...
class loss_generator(nn.Module):
    ...
    def get_heat_loss(self, output, u0, all_img, type):
        ...
        if type == 'phy':
            ...
            f_y1 = u_b[1:-1]
            f_y2 = u_u[1:-1]
            x1 = 0.5*torch.sin(4*torch.pi*y).cuda()
            x2 = 0.5*torch.sin(4*torch.pi*y).cuda()
            x1_flux = x1.unsqueeze(0).unsqueeze(0).repeat(f_x1.shape[0], 1, 1)
            x2_flux = x2.unsqueeze(0).unsqueeze(0).repeat(f_x2.shape[0], 1, 1)
            # f = torch.mean(((f_x1)**2) + ((f_x2)**2)+ ((f_y1)**2) + ((f_y2)**2))
            f = torch.mean((torch.square(f_x1-x1_flux) + torch.square(f_x2-x2_flux) \
                + torch.square(f_y1-2*torch.pi) + torch.square(f_y2-2*torch.pi)))/(10*f_x1.shape[-1])
            ...
            f = (u[0]-0.5*(torch.sin(4*torch.pi*x).cuda()+torch.sin(4*torch.pi*y).cuda()))
...
def train(model, weight_model, input, all_img, initial_state, n_iters, time_batch_size, learning_rate, 
    ...
    for epoch in range(n_iters):
        ...
        for time_batch_id in range(num_time_batch):  # num_time_batch = 1
            ...
            loss =  (f_phy**2 + f_bc**2)/(f_phy  + f_bc)
...
if __name__ == '__main__':
    ...
    # get all image
    all_imgs = []
    data_sample = pd.read_csv('./Data/Allencahn_update3_100%.csv', skiprows=0)  # read comsol generated data
    for i in range(time_steps):  # so that there is 22 time steps
        all_img = np.array(data_sample)[:,[0,1,1*i+2]]
    ...
    model_save_path =  './Allen_Cahn_equation/phycrnet_allencahn_1e-3.pt' # '/home/gli12/project/PhyCRNet/phyCRNet_AM/checkpoint_heat.pt'
