...
# input
my_data_train = genfromtxt('./Data/Allencahn_update3_100%.csv', delimiter=',', skip_header=1)
...
for epoch in range(num_epochs):
    ...
    cost4 = (((u(T_xlb, X_xlb, Y_xlb)-0.5*torch.sin(4.0 * pi * Y_xlb)) ** 2).mean() + ((u(T_xub, X_xub, Y_xub)-0.5*torch.sin(4.0 * pi * Y_xub)) ** 2).mean() \
               + ((uy(T_ylb, X_ylb, Y_ylb)-2*pi) ** 2).mean() + ((uy(T_yub, X_yub, Y_yub)-2*pi) ** 2).mean()) 
...
torch.save(model.state_dict(), './Allen_Cahn_equation/GDA_1e-4.pkl')
runnint_time = stop - start
print("training time:",runnint_time)
np.savetxt('./Allen_Cahn_equation/Adam_Training_hist_1e-3.csv', a, delimiter=",")
np.savetxt('./Allen_Cahn_equation/Adam_Forces_1e-3.csv', b, delimiter=",")
f = open('./Allen_Cahn_equation/Training_time_GDA_1e-3.txt','w')
...
plt.yscale('log')
plt.legend()
plt.savefig('./Allen_Cahn_equation/' + Mymodel + '_loss_1e-4.png')
plt.show()
plt.close()

# Evaluation
time=1.0
my_data = genfromtxt('./Data/Allencahn_update3_100%.csv', delimiter=',', skip_header=1) # using comsol datae
X = my_data[:, 0]
Y = my_data[:, 1]
Z_truth1= my_data[:,22]
Z_truth0= my_data[:,2]
...
# Convert numpy arrays to torch tensors
X= Tensor(np.reshape(X,(X.size,1))).cuda()
Y= Tensor(np.reshape(Y,(Y.size,1))).cuda()
...
model.load_state_dict(torch.load('./Allen_Cahn_equation/GDA_1e-5.pkl'))
def u(t, x, y):
    uo = model(torch.cat((t,x,y),1)).cuda()
    # if t.eq(0.0).all():
    #     uo[:,0] = u0(x,y)[:,0]
    return uo
