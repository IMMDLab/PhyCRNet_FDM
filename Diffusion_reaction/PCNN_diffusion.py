...
# my_data = genfromtxt('./Diffusion_reaction/heat equation flux.csv', delimiter=',', skip_header=1)
...
# load training data
# data 
my_data_train = genfromtxt('./Data/Diffusion_reaction_update_20%_training.csv', delimiter=',', skip_header=1)
...
def f_PDE(t, x, y):
    ...
    f1 = u_t - 0.001*(u_xx+u_yy) - (gu - gu*gu*gu -0.005 - gv)
    f2 = v_t - 0.005*(v_xx+v_yy) - (gu - gv)
...
for epoch in range(num_epochs):
    ...
    cost3 = (((uu - 0.5 * (torch.sin(8.0 * pi * X_0) + torch.cos(8.0 * pi * Y_0))) ** 2).mean() + \
                 ((vv - 0.5 * (torch.cos(8.0 * pi * X_0) + torch.sin(8.0 * pi * Y_0))) ** 2).mean())

    uxl_x, vxl_x = ux(T_xlb, X_xlb, Y_xlb)  # (tbx, xl, ybx)
    uxr_x, vxr_x = ux(T_xub, X_xub, Y_xub)  # (tbx, xr, ybx)
    uyb_y, vyb_y = uy(T_ylb, X_ylb, Y_ylb)  # (tby, xby, yl)
    uyu_y, vyu_y = uy(T_yub, X_yub, Y_yub)  # (tby, xby, yr)
    uxl, vxl = uv(T_xlb, X_xlb, Y_xlb)  # (tbx, xl, ybx)
    uxr, vxr = uv(T_xub, X_xub, Y_xub)  # (tbx, xr, ybx)
    uyb, vyb = uv(T_ylb, X_ylb, Y_ylb)  # (tby, xby, yl)
    uyu, vyu = uv(T_yub, X_yub, Y_yub)  # (tby, xby, yr)

    "Mix boundary condition"
    cost4 = 0.1*(((uxl - 0.5*torch.cos(8*pi*Y_xlb))**2) + ((vxl_x)**2) \
           + ((uxr -0.5*torch.cos(8*pi*Y_xub))**2) + ((vxr_x)**2) \
           + ((uyb_y)**2) + ((vyb - 0.5*torch.cos(8*pi*X_ylb))**2) \
           + ((uyu_y)**2) + ((vyu - 0.5*torch.cos(8*pi*X_yub))**2)).mean()
    
    cost =  (cost1 **2 + cost2 ** 2 + cost3 ** 2 +cost4**2) / (cost1 + cost2 + cost3 +cost4)
    #cost =  (cost1 **2 + cost2 **2 )/ (cost1 + cost2)
...
torch.save(model.state_dict(), './Diffusion_reaction/PCNN_Diffusion_reaction.pkl')

final_learning_rate = optimizer.param_groups[0]['lr']
print(f'Training {num_epochs} epochs, final learning rate: {final_learning_rate}')


Mymodel = 'PCNN_Diffusion_reaction'
stop = timeit.default_timer()
print("Running time: {:.2f} seconds".format(stop - start))
np.savetxt('./Diffusion_reaction/' + Mymodel + '_Training_hist.csv', a, delimiter=",")
f = open('./Diffusion_reaction/' + Mymodel + '_Training_time.txt', 'w')
...
plt.yscale('log')
plt.legend()
plt.savefig('./Diffusion_reaction/' + Mymodel + '_loss.png')
plt.show()
plt.close()
...
model.load_state_dict(torch.load('./Diffusion_reaction/PCNN_Diffusion_reaction.pkl'))
...
c[:, 1] = Y
#c[:, 2] = Z1
np.savetxt('./Diffusion_reaction/'+Mymodel +'-'+ str(time) + '.csv', c, delimiter=",")
...
plotfigure_uv(X, Y, ZU_truth0, ZV_truth0, 'Truth-Diffusion_reaction', 0.0, './Diffusion_reaction/Results/')
plotfigure_uv(X, Y, ZU_truth0_5, ZV_truth0_5, 'Truth-Diffusion_reaction', 0.5, './Diffusion_reaction/Results/')
plotfigure_uv(X, Y, ZU_truth1, ZV_truth1, 'Truth-Diffusion_reaction', 1.0, './Diffusion_reaction/Results/')
plotfigure_uv(X, Y, ZU_truth1_5, ZV_truth1_5, 'Truth-Diffusion_reaction', 1.5, './Diffusion_reaction/Results/')
plotfigure_uv(X, Y, ZU_truth2, ZV_truth2, 'Truth-Diffusion_reaction', 2.0, './Diffusion_reaction/Results/')
